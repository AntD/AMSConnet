...
proc/move_arrival_shuttle()
	...
	if (arrival_shuttle_location == 1)
		...
		a.autosay("\"Rover will leave the HQ crew department [(flying_time_from_station == 0) ? "now" : "in [flying_time_from_station] seconds."]\"", "Rover Autopilot")
	else
		fromArea = locate(/area/shuttle/arrival/station)
		a.autosay("\"Rover will leave the AMS Connet [(flying_time_from_station == 0) ? "now" : "in [flying_time_from_station] seconds."]\"", "Rover Autopilot")
...
//			return
	...
	if (start_arrival_shuttle_location == 1)
		...
		a.autosay("\"Rover left the HQ crew department. [(flying_time_to_station != 0) ? "ETA: [flying_time_to_station] sec." : ""]\"", "Rover Autopilot")
	else
		toArea = locate(/area/shuttle/arrival/pre_game)
		a.autosay("\"Rover left the AMS Connet. [(flying_time_to_station != 0) ? "ETA: [flying_time_to_station] sec." : ""]\"", "Rover Autopilot")
	fromArea = locate(/area/shuttle/arrival/spess)
	arrival_shuttle_location = 0
	while(T < flying_time_to_station)
		sleep(2)
		for(var/mob/M in toArea)
			if(M.client)
				spawn()
	...
	if(start_arrival_shuttle_location == 1)
		...
		a.autosay("\"Rover docked with the AMS Connet.\"", "Rover Autopilot")
	else
		arrival_shuttle_location = 1
		a.autosay("\"Rover docked with the the HQ crew department.\"", "Rover Autopilot")
